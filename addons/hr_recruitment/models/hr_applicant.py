...
class Applicant(models.Model):
    ...
    candidate_id = fields.Many2one('hr.candidate', index=True)
    ...
    partner_name = fields.Char(string="Applicant's Name", required=True, store=True)
    email_from = fields.Char(string="Email", store=True, readonly=False, required=True)
    ...
    profile_link = fields.Char(string="Profile Link", store=True) #Former linkedin_profile
    ...
    degree_id = fields.Many2one('hr.recruitment.degree', string="Degree", store=True)
    ...
    resume_ids = fields.Many2many(comodel_name='ir.attachment', 
                                           required=True,
                                           relation='m2m_ir_resume_rel',
                                           column1='m2m_id',
                                           column2='attachment_id',
                                           string='Resume')
    mobile_number = fields.Char(string="Mobile Number", required=True, store=True)
    secondary_number = fields.Char(string="Secondary Mobile Number", store=True)
    stages_ids = fields.Many2many('hr.recruitment.stage', string='Stages')
    stages_ref = fields.Char(string='Stages Reference', store=True)
    indeed_profile = fields.Char(string="Profile Link", store=True)
    jobstreet_profile = fields.Char(string="Jobstreet Link", store=True)
    ...
    requisition_id = fields.Many2one('hr.recruitment.requisition', string="Requisition ID", store=True)
    requisition_ref_id = fields.Many2one('hr.recruitment.requisition', string='Requisition Identifier', compute="_compute_requisition_ref_id", store=True)
    requisition_identifier_id = fields.Many2one('hr.recruitment.requisition', string='Requisition Ref', compute="_compute_requisition_ref_id", store=True)
    # requisition_ref_id = fields.Many2one('hr.requisition', string='Requisition Identifier', store=True)
    client_name_rel = fields.Many2one('hr.department', string="Client Name", related="requisition_id.department_id", store=True)
    # client_name_rel = fields.Many2one('hr.department', string="Client Name", store=True)
    job_title_rel = fields.Many2one('hr.job', string="Job Title", related="requisition_id.job_id", store=True)
    # job_title_rel = fields.Many2one('hr.job', string="Job Title", store=True)
    requisition_status_rel = fields.Selection(
        [('for_calibration', 'For Calibration'),
         ...
         ('filled', 'Filled'),
         ('completed', 'Completed')],
        string='Requisition Status', related="requisition_id.requisition_status", default='for_calibration', store=True)
    # requisition_status_rel = fields.Selection(
    #     [('for_calibration', 'For Calibration'),
    #      ('for_pooling', 'For Pooling'),
    #      ('open', 'Open'),
    ...
    priority_rel = fields.Selection([('no', 'No'), ('yes', 'Yes')], string="Prioritization", related="requisition_id.priority", store=True)
    # priority_rel = fields.Selection([('no', 'No'), ('yes', 'Yes')], string="Prioritization", store=True)
    client_classification_rel = fields.Selection([('new', 'New'), ('existing', 'Existing'), 
                                              ('sales', 'Sales'), ('support_hiring', 'Support Hiring')], string="Client Classification", related="requisition_id.client_classification", store=True)
    # client_classification_rel = fields.Selection([('new', 'New'), ('existing', 'Existing'), 
    #                                           ('sales', 'Sales'), ('support_hiring', 'Support Hiring')], string="Client Classification", store=True)
    job_classification_rel = fields.Selection([('generic', 'Generic'), ('tech', 'Tech'), 
                                           ('niche', 'Niche'), ('executive', 'Executive')], string="Job Classification", related="requisition_id.job_classification", store=True)
    # job_classification_rel = fields.Selection([('generic', 'Generic'), ('tech', 'Tech'), 
    #                                        ('niche', 'Niche'), ('executive', 'Executive')], string="Job Classification", store=True)
    support_team_rel = fields.Char(string='Support Team', related="requisition_id.support_team", store=True)
    # support_team_rel = fields.Char(string='Support Team', store=True)
    position_classification_rel = fields.Selection([('growth', 'Growth'), ('new', 'New'), 
                                           ('backfill', 'Backfill'), ('support_hiring', 'Support Hiring')], string="Position Classification", related="requisition_id.position_classification", store=True)
    # position_classification_rel = fields.Selection([('growth', 'Growth'), ('new', 'New'), 
    #                                        ('backfill', 'Backfill'), ('support_hiring', 'Support Hiring')], string="Position Classification", store=True)
    industry_rel = fields.Selection(
        ...
        string='Industry', related="requisition_id.industry", store=True)
    # industry_rel = fields.Selection(
    #     [('back office', 'Back Office'), ('customer service', 'Customer Service'), ('digital', 'Digital'),
    #      ('finance', 'Finance'), ('medical', 'Medical'), ('operations support', 'Operations Support'),
    #      ('sales', 'Sales'), ('supply chain', 'Supply Chain'), ('tech', 'Tech')], 
    #     string='Industry', store=True)
    company_rel = fields.Selection([('aiic', 'ONSITE (ISUPPORT)'), ('iswerk', 'ONSITE (ISWERK)'),
                                ('iswerk_hybrid', 'Hybrid (ISWERK)'), ('iswerk_wfh', 'WFH (ISWERK)')], string="Company", related="requisition_id.company", store=True)
    # company_rel = fields.Selection([('aiic', 'ONSITE (ISUPPORT)'), ('iswerk', 'ONSITE (ISWERK)'),
    #                             ('iswerk_hybrid', 'Hybrid (ISWERK)'), ('iswerk_wfh', 'WFH (ISWERK)')], string="Company", store=True)
    sourcing_date_rel = fields.Datetime(string='Sourcing Date', related="requisition_id.sourcing_date", store=True)
    # sourcing_date_rel = fields.Datetime(string='Sourcing Date', store=True)
    recruitment_manager_id = fields.Many2one('hr.recruitment.manager', string="Recruitment Manager", related="requisition_id.recruitment_manager_id", store=True)
    # recruitment_manager_id = fields.Many2one('hr.recruitment.manager', string="Recruitment Manager", store=True)
    channel_id = fields.Many2one('hr.recruitment.channel', string="Channel", required=True, store=True)
    specific_source_id = fields.Many2one('hr.recruitment.specific.source', string="Specific Source", compute="_compute_specific_source", readonly=False, required=True, store=True)
    job_source = fields.Selection(
        [('careers_hub', 'Careers Hub'), ('jobstreet', 'Jobstreet')],
        string='Job Source', store=True)
    referrer_name = fields.Char("Referrer's Name", store=True)
    sourcer_id = fields.Many2one('hr.recruitment.sourcer', string="Sourcer", store=True)
    lead_pipeline_id = fields.Many2one('hr.recruitment.stage', string="Lead Pipeline", domain="[('lead_pipeline', '=', True)]", store=True)
    # lead_pipeline_id = fields.Many2one('hr.recruitment.stage', string="Lead Pipeline", store=True)
    pipeline_count_id = fields.Many2one('hr.recruitment.stage', string="Pipeline Count", domain="[('pipeline_count', '=', True)]", store=True)
    # pipeline_count_id = fields.Many2one('hr.recruitment.stage', string="Pipeline Count", store=True)
    ...
    stage_owner = fields.Selection([('recruitment', 'Recruitment'), ('client', 'Client'), ('sourcing', 'Sourcing')], store=True)
    stage_set_date = fields.Datetime('Stage Set Date', store=True)
    active_file_stage_id = fields.Many2one('hr.recruitment.active.file.stage', string="Active File Stage", store=True)
    status_dropdown_id = fields.Many2one('hr.recruitment.status.selection', string="Status Selection", store=True)
    
    # Application Information
    initial_touch_date = fields.Date(string="Initial Touch Date", store=True)
    initial_touch_ageing = fields.Integer(string="Initial Touch Ageing", store=True)
    date_received = fields.Date(string="Date Received", store=True)
    month = fields.Date(string="Month", store=True)
    date_dispatched = fields.Date(string="Date Dispatched", store=True)
    days_ageing = fields.Integer(string="Days Ageing", store=True)
    movement_date = fields.Date(string="Movement Date", store=True)
    movement_date_logs = fields.Text(string="Movement Date Logs", store=True)
    forecasted_start_date = fields.Date(string="Forecasted Start Date", store=True)
    profile_reprofile_touch_date = fields.Date(string="Profile/Reprofile Touch Date", store=True)
    reprofile_identifier = fields.Boolean(string="Reprofiled?", store=True)
    reprofile_logs = fields.Text(string="Reprofile Logs", store=True)
    days_before_tapped = fields.Integer(string="Days Before Tapped", store=True)
    overall_hiring_timeline = fields.Integer(string="Overall Hiring Timeline", store=True)
    touch_logs = fields.Text(string="Touch Logs", store=True)
    applicant_url = fields.Text(string="Applicant URL", store=True)
    
    # Other Remarks
    reason_failed_interview_screening = fields.Text(string="Reason for Failed Interview or Screening", store=True)
    reason_reprofile = fields.Text(string="Reason for Reprofile", store=True)
    reason_neo_schedule_change = fields.Text(string="Reason for NEO Schedule Change", store=True)

    # Candidate Form
    survey_code = fields.Integer(string="Survey Code Number", store=True)
    old_survey_code = fields.Integer(string="Old Survey Code", store=True)
    date_survey_code_generated = fields.Datetime(string="Date Survey Code Generated", store=True)
    time_remaining = fields.Integer(string="Time Remaining")
    time_remaining_ref = fields.Integer(string="Time Remaining Reference", store=True)

    # Candidate Form - Questionaires
    ...
    # Stage Logs
    stage_logs_ids = fields.One2many('hr.recruitment.stage.logs', 'record_id', string='Stage Logs', store=True)
    ...
    # CPS Related Fields
    cps_endorsement = fields.Date(string='CPS Date Endorsement')
    # days_before_cps_endorsement = fields.Integer(string='Days Before Endorsed to CPS', compute='_compute_x_days_before_cps_endorsement')
    days_before_cps_endorsement = fields.Integer(string='Days Before Endorsed to CPS')
    cps_result = fields.Selection([('passed', 'Passed'),('failed', 'Failed'), ('waiting for feedback','Awaiting Feedback')], 'CPS Result', store=True)
    cps_result_date = fields.Date(string='CPS Result Date')
    # cps_days_before_result = fields.Integer(string='Days Before Result Was Given', compute='_compute_x_cps_days_before_result')
    cps_days_before_result = fields.Integer(string='Days Before Result Was Given')

    # Client Interview Related Fields
    ci_date = fields.Date(string='Client Interview Date')
    # days_before_ci_endorsement = fields.Integer(string='Days Before Scheduled to CI', compute='_compute_x_days_before_ci_endorsement')
    days_before_ci_endorsement = fields.Integer(string='Days Before Scheduled to CI')
    ci_result = fields.Selection(
        [('passed', 'Passed'), ('failed', 'Failed'), ('waiting for feedback', 'Awaiting Feedback')], 'Client Interview Result',
        store=True)
    ci_result_date = fields.Date(string='CI Result Date')
    # ci_days_before_result = fields.Integer(string='Days Before CI Result', compute='_compute_x_ci_days_before_result')
    ci_days_before_result = fields.Integer(string='Days Before CI Result')

    # Date Assessment Related Fields
    date_sent_assessment = fields.Date(string='Date Sent Assessment')
    # da_days_before_result = fields.Integer(string='Days Before CI Result', compute='_compute_x_da_days_before_result')
    da_days_before_result = fields.Integer(string='Days Before CI Result')

    # Second Client Interview Related Fields
    second_ci_date = fields.Date(string='2nd Client Interview Date')
    # days_before_second_ci_endorsement = fields.Integer(string='Days Before Scheduled to 2nd CI', compute='_compute_x_days_before_second_ci_endorsement')
    days_before_second_ci_endorsement = fields.Integer(string='Days Before Scheduled to 2nd CI')
    second_ci_result = fields.Selection(
        [('passed', 'Passed'), ('failed', 'Failed'), ('waiting for feedback', 'Awaiting Feedback')],
        '2nd Client Interview Result',
        store=True)
    second_ci_result_date = fields.Date(string='2nd CI Result Date')
    # second_ci_days_before_result = fields.Integer(string='Days Before 2nd CI Result', compute='_compute_x_second_ci_days_before_result')
    second_ci_days_before_result = fields.Integer(string='Days Before 2nd CI Result')

    # Third Client Interview Related Fields
    third_ci_date = fields.Date(string='3nd Client Interview Date')
    # days_before_third_ci_endorsement = fields.Integer(string='Days Before Scheduled to 3nd CI', compute='_compute_x_days_before_third_ci_endorsement')
    days_before_third_ci_endorsement = fields.Integer(string='Days Before Scheduled to 3nd CI')
    third_ci_result = fields.Selection(
        [('passed', 'Passed'), ('failed', 'Failed'), ('waiting for feedback', 'Awaiting Feedback')],
        '3nd Client Interview Result',
        store=True)
    third_ci_result_date = fields.Date(string='3nd CI Result Date')
    # third_ci_days_before_result = fields.Integer(string='Days Before 3nd CI Result', compute='_compute_x_third_ci_days_before_result')
    third_ci_days_before_result = fields.Integer(string='Days Before 3nd CI Result')

    # Job Offer Related Fields
    jo_approval_date = fields.Date(string='Sent JO Approval Date')
    jo_discussion_date = fields.Date(string='JO Discussion Date')
    jo_revert_date = fields.Date(string='JO Revert Date')
    jo_status = fields.Selection(
    ...
    # NEO Related Fields
    neo_date = fields.Date(string='NEO Date')
    neo_status = fields.Selection(
    ...
    def _compute_requisition_ref_id(self):
        for record in self:
            if record.requisition_id:
                record.requisition_ref_id = record.requisition_id.id
                record.requisition_identifier_id = record.requisition_id.id
    ...
    # Specific Source auto generate and auto remove
    @api.depends('channel_id')
    def _compute_specific_source(self):
        for record in self:
            if record.channel_id.id != 11 or record.channel_id.id != 12 or record.channel_id.id != 14:
                record.specific_source_id = False
            if record.channel_id.id == 11:
                record.specific_source_id = 43
            if record.channel_id.id == 12:
                record.specific_source_id = 44
            if record.channel_id.id == 14:
                record.specific_source_id = 48

    # Email duplicate checking to all pages
    ...
    # Mobile number duplicate checking to all pages
    ...
    def _mobile_validation(self):
        ...
        if self.mobile_number != False:
            ...
            if is_unique_referral and is_unique > 1:
                raise ValidationError(
                    f"The provided primary mobile number '{self.mobile_number}' was already used in an existing application. To avoid duplicate records, please provide a non-existent mobile number or wait till the existing application turns 90 days.")
            for rec in self:
